...
#Create a list of data to dissect
data_list <- lapply(1:50, function(i) {
...
result_2exp <- result_2exp[1:30] #correct length
result_3exp <- result_3exp[1:30] #correct length
result_4exp <- result_4exp[1:30] #correct length
...
#analyze the Reduced chi-squared
...
#plot the dissected Tau from each exponential fits
...
#after chose the ideal fit model, we now analyze the currents (A) dissected by this model

#we begin with creating other two data sets (KCND2 and KCND3 cells) by the same simulation with adjusted parameters

data_list_K2 <- lapply(1:30, function(i) {
  t<- seq(0, 15000, by = 1) # time in ms
  y_no_noise_K2 <- 1600*exp(-t/60) + 1500*exp(-t/800) + 1300*exp(-t/4000) + 400 # simulate a 3-exponential decay with errors
  error_sd_K2 <- sqrt(y_no_noise_K2) #poission error
  y_K2 <- y_no_noise_K2 + rnorm(length(t), 0, sd=error_sd_K2)
  data.frame(t = t, y = y_K2)
})

data_list_K3 <- lapply(1:30, function(i) {
  t<- seq(0, 15000, by = 1) # time in ms
  y_no_noise_K3 <- 1800*exp(-t/60) + 1500*exp(-t/800) + 1300*exp(-t/4000) + 400 # simulate a 3-exponential decay with errors
  error_sd_K3 <- sqrt(y_no_noise_K3) #poission error
  y_K3 <- y_no_noise_K3 + rnorm(length(t), 0, sd=error_sd_K3)
  data.frame(t = t, y = y_K3)
})

#create a list for results from each cell strain

result_K2 <- list()
result_K3 <- list()

#as we concluded the 3-exponential model is provided the best-fit, we only dissect these simulated data with this model

# 3 exponential fitting the simulated KCND2 data with initial guesses and boundaries

for (i in 1:length(data_list_K2)) {
  
  y <- data_list_K2[[i]]$y #y axis
  t <- data_list_K2[[i]]$t #x axis
  ...
  y_fit_K2 <- predict(fit_K2)
  
  #write parameters from 3-exp fit into the result list
  
  params <- coef(fit_K2)
  stats <- summary(fit_K2)
...
# 3 exponential fitting the simulated KCND3 data with initial guesses and boundaries

for (i in 1:length(data_list_K3)) {
  
  y <- data_list_K3[[i]]$y #y axis
  t <- data_list_K3[[i]]$t #x axis
  ...
  y_fit_K3 <- predict(fit_K3)
  
  #write parameters from 3-exp fit into the result list
  
  params <- coef(fit_K3)
  stats <- summary(fit_K3)
...
#the first component with shortest inactivation time is our target current Ito,f, here I extract the Ito,f amplitude (Amp1) from all three cells to one data set

Itof_WT <- sapply(result_3exp, function(x) x$Amp1)
Itof_K2 <- sapply(result_K2, function(x) x$Amp1)
Itof_K3 <- sapply(result_K3, function(x) x$Amp1)
Itof_table <- data.frame("Wild-type"=Itof_WT, "KCND2"=Itof_K2, "KCND3"=Itof_K3, check.names = FALSE) #check.name into false remove annoying "X" in front of any titles start with a number

Itof_list <- pivot_longer(Itof_table, cols = everything(), names_to = "Cell_Strain", values_to = "Itof")
ggplot(Itof_list, aes(x = Cell_Strain, y = Itof)) +
  geom_jitter(width = 0.05, size = 1.5, alpha = 0.5) +
  theme_minimal() +
  labs(y = "Itof Amplitude", title = "Itof current size from Wild-type, KCND2, and KCND3 murin LV Apex Epicardiums") +
  theme(legend.position = "none")

summary(aov(Itof ~ Cell_Strain, data = Itof_list))
print(TukeyHSD(aov(Itof ~ Cell_Strain, data = Itof_list),"Cell_Strain"))
